...
$('head link[rel=\'stylesheet\']')['last']()['after']('<style>.btn-m69:hover {    color: #fff;    background-color: #8b9531;    border-color: #ffffff}</style>');
                ...
                $("#profile-pic").prepend('<div id="profileShortContainer" style="margin-bottom:15px;height:27px"><button class="profileShort" value="㊣">㊣</button><button class="profileShort" value="ȚЯ">ȚЯ</button><button class="profileShort" value="Δ">Δ</button><button class="profileShort" value="ɢ">ɢ</button><button class="profileShort" value="Δ">Δ</button><button class="profileShort" value="ー">ー</button><button class="profileShort" value="❥">❥</button></div>');
        ...
        //HEAD
             ...
             //Theming Section
                ...
                panelPlusK.append("<input id='opt_shortcutProfile' type='checkbox'> Hide Profile Shortcuts<br>");
                if(JSON.parse(localStorage.getItem("opt_shortcutProfile"))===true){
                    $("#opt_shortcutProfile").prop("checked",true);
                    $("#profileShortContainer").hide();
                } else if(JSON.parse(localStorage.getItem("opt_shortcutProfile"))===false){
                    $("#opt_shortcutProfile").prop("checked",false);
                } else {
                    localStorage.setItem("opt_shortcutProfile",false);
                }
                $("#opt_shortcutProfile").change(function(){
                    if($(this).is(":checked")){
                        $("#profileShortContainer").hide();
                        localStorage.setItem("opt_shortcutProfile",true);
                    } else {
                        $("#profileShortContainer").show();
                        localStorage.setItem("opt_shortcutProfile",false);
                    }
                });
                ...
                var spektateElems = $(".btn-play.btn-primary.btn-needs-server,.btn-play-guest.btn-success.btn-needs-server,.btn-warning.btn-spectate.btn-needs-server");
                panelPlusK.append("<input id='opt_bigSpektate' type='checkbox'> Bigger Spectate<br>");
                if(JSON.parse(localStorage.getItem("opt_bigSpektate"))===true){
                    $("#opt_bigSpektate").prop("checked",true);
                        spektateElems.addClass("bigSpektate");
                } else if(JSON.parse(localStorage.getItem("opt_bigSpektate"))===false){
                    $("#opt_bigSpektate").prop("checked",false);
                } else {
                    localStorage.setItem("opt_bigSpektate",false);
                }
                $("#opt_bigSpektate").change(function(){
                    if($(this).is(":checked")){
                        spektateElems.addClass("bigSpektate");
                        localStorage.setItem("opt_bigSpektate",true);
                    } else {
                        spektateElems.removeClass("bigSpektate");
                        localStorage.setItem("opt_bigSpektate",false);
                    }
                });
                var lmmK = $("#div_score,#minimap,#minimapNode");
                panelPlusK.append('<input id="opt_leftMinimapK" type="checkbox"> Left Minimap<br>');
                if(JSON.parse(localStorage.getItem("opt_leftMinimapK"))===true){
                    $("#opt_leftMinimapK").prop("checked",true);
                        lmmK.addClass("leftMinimapK");
                } else if(JSON.parse(localStorage.getItem("opt_leftMinimapK"))===false){
                    $("#opt_leftMinimapK").prop("checked",false);
                } else {
                    localStorage.setItem("opt_leftMinimapK",false);
                }
                $("#opt_leftMinimapK").change(function(){
                    if($(this).is(":checked")){
                        lmmK.addClass("leftMinimapK");
                        localStorage.setItem("opt_leftMinimapK",true);
                    } else {
                        lmmK.removeClass("leftMinimapK");
                        localStorage.setItem("opt_leftMinimapK",false);
                    }
                });
